...
pub fn from_int(val: Int) -> List(UserFlag) {
  ...
  let flags: List(Option(UserFlag)) = {
    ...
    let flag = case val {
      val if val == staff_val -> Some(Staff)
      val if val == partner_val -> Some(Partner)
      val if val == hypesquad_val -> Some(Hypesquad)
      val if val == bug_hunter_lvevel1_val -> Some(BugHunterLevel1)
      val if val == hypesquad_online_house1_val -> Some(HypesquadOnlineHouse1)
      val if val == hypesquad_online_house2_val -> Some(HypesquadOnlineHouse2)
      val if val == hypesquad_online_house3_val -> Some(HypesquadOnlineHouse3)
      val if val == premium_early_supporter_val -> Some(PremiumEarlySupporter)
      val if val == team_pseudo_user_val -> Some(TeamPseudoUser)
      val if val == bug_hunter_level2_val -> Some(BugHunterLevel2)
      val if val == verified_bot_val -> Some(VerifiedBot)
      val if val == verified_developer_val -> Some(VerifiedDeveloper)
      val if val == certified_moderator_val -> Some(CertifiedModerator)
      val if val == bot_http_interactions_val -> Some(BotHttpInteractions)
      val if val == active_developer_val -> Some(ActiveDeveloper)
      _ -> None
    }
...
pub fn to_int(flags: List(UserFlag)) {
  let mapped_flags = {
